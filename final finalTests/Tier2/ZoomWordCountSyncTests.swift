...
//
//  Split across two files to stay under SwiftLint's type_body_length / file_length
//  caps: this file holds the shared test infrastructure plus the control, selection,
//  and forced-flush/reentrancy tests; ZoomWordCountSyncZoomedTests.swift holds the
//  zoomed-scenario tests. Same suite, same test names/counts — only which file a
//  test method lives in changed. `makeStack`, `totalWordCount`, and
//  `editBetaParagraph` are internal (not private) so the zoomed-scenario extension
//  can share them.
//
...
final class ZoomWordCountSyncTests: XCTestCase {
    ...
    /// Not private: shared with the zoomed-scenario tests in ZoomWordCountSyncZoomedTests.swift.
    func totalWordCount(_ db: ProjectDatabase, _ pid: String) throws -> Int {
    ...
    /// Not private: shared with the zoomed-scenario tests in ZoomWordCountSyncZoomedTests.swift.
    @MainActor
    func editBetaParagraph(_ webView: WKWebView) async throws {
    ...
    /// Not private: shared with the zoomed-scenario tests in ZoomWordCountSyncZoomedTests.swift.
    @MainActor
    func makeStack() async throws -> EditorStack {
    ...
    // MARK: - Mid-flight generation guard: forced flush must abandon a batch invalidated by a wholesale rewrite

    /// Regression test for the force-mode generation-bypass gap in `BlockSyncService`:
    /// a poll cycle that straddles a wholesale content rewrite (mode toggle, zoom,
    /// bibliography/notes rebuild, project switch) used to finish against a document
    /// that had been rewritten out from under it, because in force mode the two
    /// generation re-checks used to be skipped entirely — `force` was only ever meant
    /// to bypass the contentState *precondition*, not mid-flight invalidation.
    ///
    /// This forces the exact interleaving deterministically via
    /// `testAfterGenerationCaptureHook` — awaited right after the poll captures its
    /// `generationAtPoll` snapshot. (`testPollCycleHook`, used by the reentrancy test
    /// above, fires too early — before that capture — to exercise this guard: bumping
    /// the generation before that hook releases would already be reflected in the
    /// capture itself, so nothing would ever look "changed.") It gates a FORCED poll
    /// cycle immediately after its snapshot is taken, makes a real editor edit (a
    /// paragraph split, producing a block-sync INSERT — the "stale insert" this task
    /// is named for), bumps `editorState.contentState` the way a real mode
    /// toggle/zoom/rebuild does, and calls `db.replaceBlocks(...)` to stand in for
    /// that wholesale flush — all while the gated cycle is deterministically still
    /// holding its now-stale snapshot. Releasing the gate lets the cycle discover the
    /// mismatch and abandon its batch before ever touching the DB.
    @MainActor
    func testForcedFlush_abandonsStaleBatch_whenGenerationChangesMidFlight() async throws {
        let stack = try await makeStack()
        let (helper, db) = (stack.helper, stack.db)
        let (pid, sync) = (stack.pid, stack.sync)

        // Regression guard (a): explicitly wire a real EditorViewState. makeStack()'s
        // BlockSyncService never assigns `editorState`, and the guard's nil-safe
        // predicate always returns false (no generation info available) without one —
        ...
        // Regression guard (b): force: true. Unforced, the PRE-EXISTING
        // contentState == .idle guard (BlockSyncService.doPollBlockChanges) would
        // already reject this poll once contentState goes non-idle below — a false
        // pass that would say nothing about the NEW guard under test.
        let forcedTask = Task { @MainActor in
            await sync.pollBlockChangesForTest(force: true)
        }
        await gate.waitUntilReached()  // generationAtPoll captured (old value); cycle now parked

        // A real edit that produces a block-sync INSERT (paragraph split via Enter) —
        // the "stale insert" this task is named for. Mirrors the cursor-placement
        // technique in testZoomed_newBlockWhileZoomed_reachesDatabase.
        _ = try await helper.webView.evaluateJavaScript(
            """
            (() => {
        ...
        // Simulate the wholesale rewrite (mode toggle / zoom / bibliography rebuild /
        // project switch) landing while the forced poll is genuinely mid-flight: its
        // generationAtPoll snapshot was already taken (captured before the gate), and
        // this rewrite happens strictly after — the exact race the guard exists to close.
        let rewrittenMarkdown = "# Alpha\n\nRewritten entirely.\n\n# Beta\n\nAlso rewritten.\n"
        let rewrittenBlocks = BlockParser.parse(markdown: rewrittenMarkdown, projectId: pid)
            .sorted { $0.sortOrder < $1.sortOrder }
        editorState.contentState = .editorTransition
        try db.replaceBlocks(rewrittenBlocks, for: pid)

        gate.open()
        await forcedTask.value

        // Assert on something that specifically proves the NEW guard caused the
        // rejection, not incidental behavior elsewhere.
        XCTAssertGreaterThan(
            sync.testGenerationAbandonCount, 0,
            "the mid-flight generation guard must have fired and abandoned the stale batch"
        )

        let afterBlocks = try TestFixtureFactory.fetchBlocks(from: db)
            .sorted { $0.sortOrder < $1.sortOrder }
        XCTAssertEqual(
            afterBlocks.count, rewrittenBlocks.count,
            "no extra row from the stale insert must have been appended on top of the rewrite " +
                "(expected=\(rewrittenBlocks.count), got=\(afterBlocks.count))"
        )
        XCTAssertEqual(
            afterBlocks.map(\.textContent), rewrittenBlocks.map(\.textContent),
            "the document must match exactly what db.replaceBlocks(...) wrote, with no stale " +
                "insert content mixed in"
        )
    }
}
