...
import { isHistoryTransaction, redo, undo } from '@milkdown/kit/prose/history';
import type { Node } from '@milkdown/kit/prose/model';
import type { EditorState, Transaction } from '@milkdown/kit/prose/state';
import type { EditorView } from '@milkdown/kit/prose/view';
import { hasPendingChanges } from './block-sync-plugin';
...
export interface UndoRegistryEntry {
  /** Full EditorState captured at op time -- swapped in wholesale on structural undo
   * (checkpoint-swap, plan §4.3). Unused in Phase 2 (nothing reads it yet). */
...
/** blockSync's committed pending-change queue (updates/inserts/deletes not yet flushed to
 * Swift) -- cheap and synchronous, matching plan §4.2's "closes the unflushed-delta race". */
