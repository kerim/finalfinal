...
final class DocumentManager {
    ...
    func exportBlocks() async throws -> [Block] {
        ...
        return blocks.filter { !$0.isBibliography }
    }

    /// Load content for export, excluding bibliography blocks.
    /// See `exportBlocks()` for the flush-before-read behavior.
    func loadContentForExport() async throws -> String? {
        let exportBlocks = try await exportBlocks()
