...
/// Compute backup analysis for a given comparison mode, current sections, and snapshot sections
func computeBackupAnalysis(
    snapshotSections: [SnapshotSection],
    currentSections: [SnapshotSectionViewModel],
    previousSections: [SnapshotSection],
    comparisonMode: ComparisonMode
) -> (changes: [String: SectionChangeType], wordDeltas: [String: Int]) {
    let displayed = snapshotSections.map { SnapshotSectionViewModel(from: $0) }
    let comparison: [SnapshotSectionViewModel]
    switch comparisonMode {
    case .vsCurrent:
        comparison = currentSections
    case .vsPrevious:
        comparison = previousSections.map { SnapshotSectionViewModel(from: $0) }
    }
