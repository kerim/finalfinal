...
struct VersionListView: View {
    ...
    /// Precomputed snapshot index lookup (avoids O(n) search per row)
    private var snapshotIndexMap: [String: Int] {
        Dictionary(uniqueKeysWithValues: snapshots.enumerated().map { ($1.snapshot.id, $0) })
    }
                        ...
                        SwiftUI.Section(header: Text(group.title).font(.caption).foregroundStyle(themeManager.currentTheme.sidebarTextSecondary)) {
                            ForEach(group.snapshots) { item in
                                SnapshotRowView(
                                    ...
                                    snapshotIndex: snapshotIndexMap[item.snapshot.id] ?? 0,
...
struct SnapshotRowView: View {
    ...
    private var wordDelta: Int? {
        ...
        case .vsPrevious:
            guard snapshotIndex + 1 < allSnapshots.count else { return nil }
            let olderSnapshotId = allSnapshots[snapshotIndex + 1].snapshot.id
    ...
    private var sectionDelta: Int? {
        ...
        case .vsPrevious:
            guard snapshotIndex + 1 < allSnapshots.count else { return nil }
            let olderMarkdown = allSnapshots[snapshotIndex + 1].snapshot.previewMarkdown
            ...
            HStack(spacing: 6) {
                ...
                if let sDelta = sectionDelta {
                    Text(sDelta > 0 ? "+\(sDelta) sections" : "\(sDelta) sections")
                        .font(.caption.weight(.medium))
                        .foregroundStyle(themeManager.currentTheme.statusColors.deltaColor(for: sDelta))
                }
