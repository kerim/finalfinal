...
final class EditorSchemeHandler: NSObject, WKURLSchemeHandler {
    ...
    func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        ...
        guard let fileURL = bundleURL(for: url) else {
            DebugLog.log(.scheme, "[EditorSchemeHandler] File not found: \(url)")
            urlSchemeTask.didFailWithError(SchemeError.fileNotFound)
            return
        }

        guard let data = try? Data(contentsOf: fileURL) else {
            DebugLog.log(.scheme, "[EditorSchemeHandler] Failed to read: \(fileURL)")
        ...
        DebugLog.log(.scheme, "[EditorSchemeHandler] Served: \(url.path) (\(mimeType), \(data.count) bytes)")
    }
