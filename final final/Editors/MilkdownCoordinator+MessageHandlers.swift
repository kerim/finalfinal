...
extension MilkdownEditor.Coordinator {
    ...
    /// Handle openEquationDialog message from JS.
    /// Shows an NSAlert with inline/display toggle + LaTeX text field.
    /// On OK, calls window.FinalFinal.insertEquation(latex, isDisplay).
    @MainActor
    func handleOpenEquationDialog() {
        ...
        alert.beginSheetModal(for: window) { [weak self] response in
            ...
            self?.webView?.evaluateJavaScript(js) { _, error in
                if let error {
                    DebugLog.log(.editor, "[MilkdownEditor] insertEquation JS error: \(error)")
                }
            }
        }
    }
