//
//  MilkdownCoordinator+MessageDispatch.swift
//  final final
//
//  WKScriptMessageHandler entry point for MilkdownEditor.Coordinator, split out
...
//  low. A flat `switch message.name` over all 17 message names scores far too
...
extension MilkdownEditor.Coordinator {
    ...
    nonisolated func handleCitationMessage(_ message: WKScriptMessage) -> Bool {
        ...
        case "resolveCitekeys":
            guard let citekeys = message.body as? [String] else { return true }
            Task { @MainActor in
    ...
    /// Milkdown deliberately omits the `onContentChange` call here — CodeMirror's counterpart
    /// calls it (and reads the raw, anchor-bearing content); that divergence is intentional
    /// (Milkdown has no anchor-stripping step), not a bug to unify.
    @MainActor
