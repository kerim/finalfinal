...
## Greedy Document-Order Matching Fails Near Structural Boundaries

**Problem:** Pressing Enter near the bibliography boundary corrupted bibliography entries. The new empty paragraph stole a bibliography entry's block ID, causing `detectChanges()` to report the bibliography entry as having empty content → database corruption.

**Root Cause:** `assignBlockIds()` processed blocks in document order and used greedy proximity matching. When a new paragraph appeared at pos 30 and the bibliography entry's old position was at pos 44, the new paragraph claimed that ID (distance=14) before the real bibliography entry at pos 46 could claim it (distance=2).

**Solution:** Two-phase proximity matching when `structureChanged` is true:

1. **Phase 1 — Exact-position matches:** Same as before. If a block's position has an existing ID with matching type, claim it.
2. **Phase 2 — Closest-first global matching:** Collect ALL (deferredBlock, candidate) pairs with distances, sort by distance ascending, then assign greedily. Closest pairs win regardless of document order.

```typescript
// Phase 2: collect all pairs, sort by distance, assign closest-first
const pairs = [];
for (const d of deferred) {
  for (const [oldPos, id] of existingIds) {
    if (!claimedIds.has(id) && existingTypes.get(oldPos) === d.nodeType) {
      const distance = Math.abs(oldPos - d.offset);
      if (distance < 500) pairs.push({ newOffset: d.offset, oldPos, id, distance });
    }
  }
}
...
**General principle:** When matching nodes across document edits by proximity, document-order greedy matching creates a "first come, first served" problem — a new node can steal an ID from a neighbor before the rightful owner gets a chance. Global closest-first matching resolves this by ensuring the best match always wins.

---

## Machine-Generated Content Needs Write Protection

**Problem:** Even with correct ID matching, bibliography content could be corrupted if any future bug in the ID assignment or sync pipeline caused an UPDATE to target a bibliography block.

**Solution:** Defense-in-depth guard in `Database+Blocks.applyBlockChangesFromEditor`: skip any update where `block.isBibliography == true`. Bibliography content is machine-generated by `BibliographySyncService` and should never be modified through the editor polling pipeline.

```swift
for update in changes.updates {
    if var block = try Block.fetchOne(db, key: update.id) {
        if block.isBibliography {
            continue  // Never overwrite bibliography via editor sync
        }
        // ... normal update logic
    }
}
...
**General principle:** When content is managed by a dedicated service (bibliography, footnote definitions, auto-generated headers), protect it from modification by the general-purpose sync pipeline. A simple type/flag guard prevents an entire class of corruption bugs regardless of upstream logic quality.
...
**Solution:** Call `pollBlockChangesNow()` before any `fetchBlocksWithIds()` in bibliography, notes, and footnote rebuild handlers. This force-flushes pending JS changes to the database before reading.

```swift
// In bibliography notification handler:
await blockSyncService.pollBlockChangesNow()  // Flush pending JS → DB
guard let result = fetchBlocksWithIds() else { ... }
await blockSyncService.setContentWithBlockIds(...)
```

**General principle:** In a polling-based sync architecture, any operation that reads from the database and pushes back to the editor must first flush pending editor changes. Otherwise the read returns stale data, and the pending changes create duplicates when they eventually flush.
