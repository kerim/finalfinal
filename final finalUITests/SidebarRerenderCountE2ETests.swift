...
//
//  Ports the measurement approach from `final finalUITests/E2EScratchTests.swift` (that file's
//  own disposable investigation run -- see its header for the full story of how an earlier,
//  un-deduped diagnostic-log reader produced a false pass) into a permanent, committed test.
//  Two changes from that disposable predecessor matter for correctness, not just permanence:
//
...
//      assertion (SidebarBody / ContentViewBody, round-3 must-fix-4 -- see its own comment below
//      for why the denominator is ContentViewBody, not raw keystrokes) is a SAME-LOG ratio, so a
...
//      That is NOT what the dedupe fix protects there; ContentViewBody isn't causally downstream
//      of OutlineSidebar's own fix the way the original investigation's WordCountLabel-based
//      ratio was (see E2EScratchTests.swift's header for that confound), so this ratio stays
//      meaningful either way. What the dedupe fix DOES protect: the POSITIVE CONTROL and the
//      `keystrokes` sanity gates below compare a log-derived count against the externally-known,
//      NOT-read-from-the-log `keystrokes` value -- a tripled count would corrupt exactly those
//      comparisons. And more fundamentally, every raw count in the UNCONDITIONALLY printed/
//      attached `summary` (this file's whole verification-integrity purpose -- see
//      E2EScratchTests.swift's header for the incident this exists to never repeat) needs to
//      reflect ground truth, not a 3x-inflated fiction, for a human reading that evidence later
//      to draw the right conclusion from it.
//
...
//     gate depend on (the MAIN ratio's own denominator is `ContentViewBody`, not `keystrokes` --
//     see round-3 must-fix-4's comment below -- so heavy character loss moves those two gates,
//     not the main ratio itself), independent of any sidebar behavior.
...
final class SidebarRerenderCountE2ETests: XCTestCase {
    ...
    /// this suite's established per-file convention (see e.g. `E2EScratchTests.swift`, which
    /// duplicates its own copy for the identical reason).
    ...
    // MARK: - Diagnostic log delta helpers (ported from E2EScratchTests.swift; log READING itself
    // is not duplicated here -- it calls through to UnifiedUndoE2ETests's FIXED, deduped statics)
    ...
    struct ViewUpdateCounts {
        ...
        var total: Int { contentViewBody + sidebarBody + wordCountLabel }
    }

    /// Counts occurrences of each bracketed marker substring in a diagnostic-log delta. See
    /// `E2EScratchTests.countViewUpdateMarkers`'s doc comment for why a plain substring count is
    /// exact here (each marker is a single, distinct bracket tag that never appears elsewhere in
    /// this app's DebugLog output, and DiagnosticLogFile.append() never dedupes repeated lines).
    ...
    /// Types one character at a time until `duration` has elapsed. Ported verbatim (rationale
    /// unchanged) from `E2EScratchTests.typeContinuously`: a deadline-driven loop so VM slowness
    /// stretches the keystroke COUNT rather than shrinking the measurement WINDOW, and content is
    /// throwaway (this measures re-render counts, not resulting text) so `typeText`'s known
    /// character-drop flakiness under VM load doesn't corrupt what's being measured -- using
    ...
    private func runTypingPhase(name: String, findElement: () -> XCUIElement?) {
        ...
        // -- see E2EScratchTests.swift's header for the original incident).
        let summary = "[E2E-VIEWUPDATES] phase=\(name) keystrokes=\(keystrokes) " +
            ...
            "WordCountLabel=\(counts.wordCountLabel)"
        print(summary)
        ...
        XCTAssertGreaterThan(
            counts.total, 0,
            "Phase \"\(name)\": ContentViewBody + SidebarBody + WordCountLabel was 0 -- likely " +
            "causes: wrong log path, diagnostics off, stale offsets, or log rotation (see this " +
            "test's file header for the full confounder list). \(summary)"
        )
        ...
        // ContentView.body pass corresponds to (at least roughly) one keystroke reaching the
        // editor and propagating back to SwiftUI state, so a healthy run should show
        // ContentViewBody firing on well over half the typed keystrokes.
        XCTAssertGreaterThan(
            counts.contentViewBody, keystrokes / 2,
            "Phase \"\(name)\": ContentViewBody (\(counts.contentViewBody)) should track keystrokes " +
        ...
        // fraction of ContentView's own re-renders -- not close to 1:1. Divided by the MEASURED
        // `contentViewBody` count, not the test-counted `keystrokes` -- round-3 review finding:
        // those two denominators can diverge (e.g. under significant typeText character-drop
        // loss), and a ratio against `keystrokes` alone could false-pass in a narrow band where
        // keystroke loss and a partially-broken re-render rate happened to offset each other. The
        // positive-control assertion above (against `keystrokes`) stays as a separate sanity gate
        // that keystrokes actually landed; this one measures the actual invalidation coupling
        // between the two counters the log itself recorded. This bound (0.15) is only meaningful
        // once the §3 diagnostic-log dedupe fix has landed (it has, in this same round) -- see
        // this file's header for why an un-deduped log would make this number meaningless in
        // either direction.
        if counts.contentViewBody > 0 {
            let ratio = Double(counts.sidebarBody) / Double(counts.contentViewBody)
            XCTAssertLessThan(
                ratio, 0.15,
                "Phase \"\(name)\": SidebarBody (\(counts.sidebarBody)) should stay a small fraction " +
                "of ContentViewBody (\(counts.contentViewBody)), not track it close to 1:1. " +
